...
def get_similarity(mk: torch.Tensor,
    ...
    if ms is not None:
        similarity = similarity * ms / (math.sqrt(CK)+1e-12)  # B*N*HW
    else:
        similarity = similarity / (math.sqrt(CK)+1e-12)  # B*N*HW
...
def do_softmax(
    ...
    if top_k is not None:
        ...
        x_exp = values.exp_()
        x_exp /= (torch.sum(x_exp, dim=1, keepdim=True)+1e-12)
        if inplace:
        ...
        affinity = x_exp / (x_exp_sum+1e-12)
        indices = None
