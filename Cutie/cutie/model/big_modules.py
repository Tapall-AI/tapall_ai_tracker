...
from cutie.model.utils import convnextv2
...
class PixelEncoder(nn.Module):
    def __init__(self, model_cfg: DictConfig):
        ...
        if self.is_resnet:
            ...
            elif model_cfg.pixel_encoder.type == 'resnet101':
                # network = resnet.resnet101(pretrained=True)
                network = torch.hub.load('pytorch/vision:v0.10.0', 'wide_resnet101_2', pretrained=True)
            else:
            ...
            if model_cfg.pixel_encoder.type == 'convnextv2-l':
                network = convnextv2.convnextv2_large(pretrained=True, in_chans=3)
            elif model_cfg.pixel_encoder.type == 'convnextv2-b':
                network = convnextv2.convnextv2_base(pretrained=True, in_chans=3)
            else:
                raise NotImplementedError
            self.ds1 = network.downsample_layers[0]
            self.ds2 = network.downsample_layers[1]
            self.ds3 = network.downsample_layers[2]
            self.layer1 = network.stages[0]
            self.layer2 = network.stages[1]
            self.layer3 = network.stages[2]
    ...
    def forward(self, x: torch.Tensor) -> (torch.Tensor, torch.Tensor, torch.Tensor):
        if self.is_resnet:
            ...
            f4 = self.ds1(x)
            f4 = self.layer1(f4)
            f8 = self.ds2(f4)       # from float32 to float16 since AMP
            f8 = self.layer2(f8)
            f16 = self.ds3(f8)
            f16 = self.layer3(f16)
...
class MaskEncoder(nn.Module):
    def __init__(self, model_cfg: DictConfig, single_object=False):
        ...
        elif model_cfg.mask_encoder.type == 'convnextv2-l':
            self.is_resnet = False
            network = convnextv2.convnextv2_large(pretrained=True, in_chans=3+extra_dim)
        elif model_cfg.mask_encoder.type == 'convnextv2-b':
            self.is_resnet = False
            network = convnextv2.convnextv2_base(pretrained=True, in_chans=3+extra_dim)
        elif model_cfg.mask_encoder.type == 'convnextv2-n':
            self.is_resnet = False
            network = convnextv2.convnextv2_nano(pretrained=True, in_chans=3+extra_dim)
        else:
            raise NotImplementedError
    ...
    def forward(self,
        ...
        for i in range(0, num_objects, chunk_size):
            ...
            if self.is_resnet:
                g_chunk = self.conv1(g_chunk)                   # bs, 4, 480, 480
                ...
                g_chunk = self.layer3(g_chunk)  # 1/16          # bs, 256, 30, 30
            else:
                g_chunk = self.ds1(g_chunk)
                g_chunk = self.layer1(g_chunk)
                g_chunk = self.ds2(g_chunk)
                g_chunk = self.layer2(g_chunk)
                g_chunk = self.ds3(g_chunk)
