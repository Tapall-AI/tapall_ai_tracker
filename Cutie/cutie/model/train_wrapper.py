...
class CutieTrainWrapper(CUTIE):
    ...
    def forward(self, data: Dict):
        ...
        # with torch.cuda.amp.autocast(enabled=self.use_amp): # when True, may cause 'nan' losses
            ...
            # if torch.isnan(sensory).any():
            #     try:
            #         log.info(f'max shrinkages: {shrinkages.max().item()}')
            #         log.info(f'max selections: {selections.max().item()}')
            #         log.info(f'max ms_feat: {ms_feat.max().item()}')
            #         log.info(f'max pix_feat: {pix_feat.max().item()}')
            #     except:
            #         pass

            # log.info(f'sensory: {sensory.max().item()}')
            ...
            for ti in range(1, seq_length):
                ...
                #     try:
                #         log.info(f'max selections: {selections.max().item()}')
                #         log.info(f'max ref_shrinkages: {ref_shrinkages.max().item()}')
                #         log.info(f'max ms_feat: {ms_feat.max().item()}')
                #         log.info(f'max pix_feat: {pix_feat.max().item()}')
                #     except:
                #         pass
                # log.info(f'max readout: {readout.max().item()}')
